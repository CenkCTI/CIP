...
describe("protected transport, cleanup, redirects and compression",()=>{
 ...
 it.each(["application/feed+json","application/json","text/plain"])("accepts JSON Feed media type %s",async contentType=>expect((await fetchFeed("https://example.com/feed",{}, {resolver,transport:async()=>response(200,{"content-type":contentType},JSON.stringify({version:"https://jsonfeed.org/version/1.1",title:"x",items:[]}))})).status).toBe(200));
 it("rejects media/body mismatches",async()=>{await expect(fetchFeed("https://example.com",{}, {resolver,transport:async()=>response(200,{"content-type":"application/json"},rss)})).rejects.toMatchObject({code:"CONTENT_TYPE_REJECTED"});await expect(fetchFeed("https://example.com",{}, {resolver,transport:async()=>response(200,{"content-type":"application/xml"},"{}") })).rejects.toMatchObject({code:"CONTENT_TYPE_REJECTED"});});
