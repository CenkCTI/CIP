...
describe("migration 023 contract",()=>{
 ...
 it("binds leases to run, URL hash, token and expiry",()=>{expect(sql).toContain("fetch_lease_run_id");expect(sql).toContain("fetch_lease_url_hash");expect(sql).toContain("lease_token_hash");expect(sql).toContain("LEASE_EXPIRED");});
