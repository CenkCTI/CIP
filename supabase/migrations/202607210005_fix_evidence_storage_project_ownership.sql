-- Record the manually verified production fix for Evidence Storage signed-upload RLS.
...
create or replace function public.evidence_storage_project_is_owned(
...
revoke all
on function public.evidence_storage_project_is_owned(text)
from public, anon;
...
using (
  ...
  and public.evidence_storage_project_is_owned(
...
create policy "evidence storage update own"
on storage.objects
for update
to authenticated
using (
