...
const pulseIdSchema=z.string().regex(/^[0-9a-f]{24}$/),searchQuerySchema=z.string().min(2).max(120);
export type OtxSearchState={results:OtxPulseSearchSummary[];error?:string};
const searchFailure=(error:unknown)=>error instanceof OtxError?({OTX_AUTH_FAILED:"AlienVault OTX rejected the stored API key.",OTX_RATE_LIMITED:"AlienVault OTX rate limited the search. Try again later.",OTX_TIMEOUT:"AlienVault OTX search timed out.",OTX_NETWORK_FAILURE:"AlienVault OTX could not be reached.",OTX_INVALID_RESPONSE:"AlienVault OTX returned an invalid search response."}as Partial<Record<string,string>>)[error.code]??"AlienVault OTX search could not be completed safely.":"AlienVault OTX search could not be completed safely.";
