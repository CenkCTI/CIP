...
import { claimIocConnection,completeIocIngestion,discardLegacyOtxContinuation,ensureSyntheticConnection,failIocIngestion,setIocConnectionEnabled,type IocClaim } from "@/lib/ioc-connectors/trusted-workflow-client";
...
import {hasActiveOtxContinuation,otxContinuationScope,parseOtxCursor} from "@/lib/ioc-connectors/providers/otx/cursor";
...
import {detectExactOtxIndicator,fetchOtxPulse,lookupOtxIndicator,searchOtxPulses,type OtxPulseSearchSummary} from "@/lib/ioc-connectors/providers/otx/client";
import {buildOtxResult} from "@/lib/ioc-connectors/providers/otx/adapter";
import {adapterResultSchema} from "@/lib/ioc-connectors/schema";
import {safeIocMessage} from "@/lib/ioc-connectors/errors";
...
const pulseIdSchema=z.string().regex(/^[0-9a-f]{24}$/),searchQuerySchema=z.string().trim().min(2).max(120);
export type OtxSearchState={results:OtxPulseSearchSummary[];error?:string;query?:string;page?:number;hasMore?:boolean;mode?:"GLOBAL"|"EXACT";reputation?:number|null;references?:string[]};
const searchFailure=(error:unknown)=>error instanceof OtxError?safeIocMessage(error.code):"AlienVault OTX search could not be completed safely.";
export async function searchOtxPulseAction(connectionId:string,previous:OtxSearchState,form:FormData):Promise<OtxSearchState>{const loadingMore=form.get("intent")==="load_more",raw=loadingMore?previous.query:form.get("query"),query=searchQuerySchema.safeParse(raw);if(!idSchema.safeParse(connectionId).success||!query.success)return{results:[],error:"Enter an OTX Pulse search term between 2 and 120 characters."};const page=loadingMore?(previous.page??1)+1:1;if(page>5)return previous;try{const{user,supabase}=await requireUser(),{data}=await supabase.from("ioc_provider_connections").select("id,provider_key,enabled,archived_at").eq("owner_id",user.id).eq("id",connectionId).maybeSingle();if(!data||data.provider_key!=="ALIENVAULT_OTX"||!data.enabled||data.archived_at)return{results:[],error:"The AlienVault OTX connection is unavailable."};const credential=await loadCredential(user.id,connectionId,"ALIENVAULT_OTX");if(!credential)return{results:[],error:"AlienVault OTX credential is missing."};const exact=!loadingMore?detectExactOtxIndicator(query.data):null;if(exact){const result=await lookupOtxIndicator(credential,query.data);return{results:result.pulses,query:query.data,page:1,hasMore:false,mode:"EXACT",reputation:result.reputation,references:result.references}}const next=await searchOtxPulses(credential,query.data,page),results=loadingMore?[...previous.results,...next]:next;return{results:results.slice(0,25),query:query.data,page,hasMore:next.length===5&&page<5,mode:"GLOBAL"}}catch(error){return{...previous,error:searchFailure(error)}}}
export async function importOtxPulse(connectionId:string,pulseId:string):Promise<IocActionResult>{if(!idSchema.safeParse(connectionId).success||!pulseIdSchema.safeParse(pulseId).success)return safe;let claim:IocClaim|undefined;try{const{user,supabase}=await requireUser(),[{data:connection},{data:stored}]=await Promise.all([supabase.from("ioc_provider_connections").select("id,provider_key,enabled,archived_at").eq("owner_id",user.id).eq("id",connectionId).maybeSingle(),supabase.from("ioc_provider_cursors").select("cursor_value").eq("owner_id",user.id).eq("provider_connection_id",connectionId).maybeSingle()]);if(!connection||connection.provider_key!=="ALIENVAULT_OTX"||!connection.enabled||connection.archived_at)return safe;const scope=otxContinuationScope(stored?.cursor_value??null),existing=parseOtxCursor(stored?.cursor_value??null);if(scope==="LEGACY_BULK")return{error:"Discard the pending legacy bulk OTX snapshot before importing a Pulse."};if(scope==="SEARCH_PULSE"&&existing?.continuation?.pulse_id!==pulseId)return{error:"Finish the current Pulse import before selecting another Pulse."};const claimed=await claimIocConnection(user.id,connectionId,"MANUAL");claim=claimed.data?.[0];if(claimed.error||!claim)return{error:"An OTX import is already running."};const credential=await loadCredential(user.id,connectionId,"ALIENVAULT_OTX");if(!credential)throw new OtxError("OTX_CREDENTIAL_REQUIRED");const raw=await fetchOtxPulse(credential,pulseId),parsed=adapterResultSchema.safeParse(buildOtxResult([raw],claim.cursor_value,{now:new Date(),scope:"SEARCH_PULSE",pulseId}));if(!parsed.success)throw new Error("ADAPTER_RESULT_CONTRACT_INVALID");const result=parsed.data,{error}=await completeIocIngestion({p_owner_id:claim.owner_id,p_connection_id:claim.connection_id,p_run_id:claim.run_id,p_lease_token:claim.lease_token,p_status:result.status,p_starting_cursor_version:claim.cursor_version,p_next_cursor:result.nextCursor??null,p_items:result.items});if(error)throw new Error("IOC_COMPLETION_FAILED");refresh();return{success:`Pulse batch completed: ${result.diagnostics.mapped_count} candidates processed, ${result.diagnostics.mapping_skipped_count} records skipped; ${result.diagnostics.deferred_count} remain.`}}catch(error){if(claim)await failIocIngestion({p_owner_id:claim.owner_id,p_connection_id:claim.connection_id,p_run_id:claim.run_id,p_lease_token:claim.lease_token,p_error_code:error instanceof OtxError?error.code:"PROVIDER_INTERNAL_FAILURE",p_error_message:error instanceof OtxError?safeIocMessage(error.code):safeIocMessage("PROVIDER_INTERNAL_FAILURE")});return{error:error instanceof OtxError?safeIocMessage(error.code):safeIocMessage("PROVIDER_INTERNAL_FAILURE")}}}
export async function discardPendingOtxBulkSnapshot(connectionId:string,form:FormData):Promise<IocActionResult>{if(!idSchema.safeParse(connectionId).success||form.get("confirmation")!=="DISCARD")return{error:"Confirm that only the pending bulk snapshot should be discarded."};try{const{user}=await requireUser(),{error}=await discardLegacyOtxContinuation(user.id,connectionId);if(error)return safe;refresh();return{success:"Pending legacy bulk OTX snapshot discarded. Imported IOC history was preserved."}}catch{return safe}}
...
export async function syncIocProviderConnection(connectionId: string): Promise<IocActionResult> {
  try {
    ...
    if (data.provider_key === "ALIENVAULT_OTX") return { error: "Use OTX Search & Import to select one Pulse." };
