...
import {fetchOtxPulse,searchOtxPulses,type OtxPulseSearchSummary} from "@/lib/ioc-connectors/providers/otx/client";
...
export type OtxSearchState={results:OtxPulseSearchSummary[];error?:string};
const searchFailure=(error:unknown)=>error instanceof OtxError?({OTX_AUTH_FAILED:"AlienVault OTX rejected the stored API key.",OTX_RATE_LIMITED:"AlienVault OTX rate limited the search. Try again later.",OTX_TIMEOUT:"AlienVault OTX search did not finish within 30 seconds. Try a more specific term.",OTX_NETWORK_FAILURE:"AlienVault OTX could not be reached.",OTX_INVALID_RESPONSE:"AlienVault OTX returned an invalid search response."}as Partial<Record<string,string>>)[error.code]??"AlienVault OTX search could not be completed safely.":"AlienVault OTX search could not be completed safely.";
export async function searchOtxPulseAction(connectionId:string,_previous:OtxSearchState,form:FormData):Promise<OtxSearchState>{const query=searchQuerySchema.safeParse(form.get("query"));if(!idSchema.safeParse(connectionId).success||!query.success)return{results:[],error:"Enter an OTX Pulse search term between 2 and 120 characters."};try{const{user,supabase}=await requireUser(),{data}=await supabase.from("ioc_provider_connections").select("id,provider_key,enabled,archived_at").eq("owner_id",user.id).eq("id",connectionId).maybeSingle();if(!data||data.provider_key!=="ALIENVAULT_OTX"||!data.enabled||data.archived_at)return{results:[],error:"The AlienVault OTX connection is unavailable."};const credential=await loadCredential(user.id,connectionId,"ALIENVAULT_OTX");if(!credential)return{results:[],error:"AlienVault OTX credential is missing."};return{results:await searchOtxPulses(credential,query.data)}}catch(error){return{results:[],error:searchFailure(error)}}}
