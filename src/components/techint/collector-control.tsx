  ...
  return (
    <section className="card panel-corners space-y-4 border-l-2 border-l-amber-500/60">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="citem-eyebrow">Phase 2.3F-A/B/C/D / Collection, history, semantics &amp; anomaly baselines</p>
          <h2 className="citem-section-title mt-1">Desktop collector</h2>
          <p className="mt-2 max-w-3xl text-sm text-stone-400">
            The desktop companion owns the long-running collection loop even when the browser is closed. It claims one due source and drains it through bounded server work units, then requests rate-gated history and deterministic anomaly maintenance. Provider API keys and the Supabase service role remain server-side.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <Link className="citem-button-ghost" href="/techint/sources/semantics">Source semantics</Link>
          <Link className="citem-button-ghost" href="/techint/sources/history">History diagnostics</Link>
          <Link className="citem-button-ghost" href="/techint/sources/anomalies">Anomaly diagnostics</Link>
      ...
      <p className="text-xs text-stone-500">
        Catch-up is source-dependent: cursor/window sources can recover bounded offline gaps; snapshot-only sources recover current state but cannot reconstruct every intermediate state that the upstream provider no longer exposes. Intermediate bounded work never advances the authoritative source cursor. Historical rollups and anomaly evaluations are derived and recomputable; collection coverage is tracked separately so a missing or failed collection period is never presented as zero activity or a volume drop.
      </p>
    </section>
  );
}