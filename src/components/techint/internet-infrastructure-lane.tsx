...
function time(value: string | null | undefined) { return value ? new Date(value).toLocaleString() : "Unknown"; }
function statusText(value: string | null | undefined) { return value ?? "UNKNOWN"; }
function MetricCard({ label, value, note }: { label: string; value: string; note: string }) { return <article className="card panel-corners"><p className="citem-eyebrow">{label}</p><p className="mt-2 text-2xl font-semibold text-stone-100">{value}</p><p className="mt-2 text-xs leading-5 text-stone-500">{note}</p></article>; }
...
export function InternetInfrastructureLane({series,status,statusUnavailable=false,measurementsUnavailable=false}:{series:readonly NodeMeasurementSeries[];status?:NodeRoutingStatus;statusUnavailable?:boolean;measurementsUnavailable?:boolean;}) {
  ...
  const updateTotal=completeAdditiveTotal(updates),announcementTotal=completeAdditiveTotal(announcements),withdrawalTotal=completeAdditiveTotal(withdrawals);
  return <section className="space-y-4" aria-labelledby="internet-infrastructure-heading">
    <div><p className="citem-eyebrow">Lane 03</p><h2 id="internet-infrastructure-heading" className="citem-section-title">Internet Infrastructure</h2><p className="mt-2 max-w-4xl text-sm leading-6 text-stone-500">Internet routing observations collected by BAYKUSH Intelligence Node from RIPE NCC Routing Information Service (RIS). These measurements describe observed BGP activity; they are not attack, outage, hijack, attribution, or global-Internet verdicts.</p></div>
    {measurementsUnavailable?<p className="rounded border border-stone-800 px-3 py-2 text-sm text-stone-500">Historical routing measurement series are temporarily unavailable. Current routing operational status below remains an independent fact and does not manufacture missing historical values.</p>:null}
    ...
    <section className="space-y-3"><div><p className="citem-eyebrow">Routing activity</p><h3 className="text-sm font-semibold text-stone-200">Materialized activity series</h3></div><div className="grid gap-4 xl:grid-cols-3"><SeriesPanel title="BGP UPDATE messages" series={updates}/><SeriesPanel title="Announcement prefix events" series={announcements}/><SeriesPanel title="Withdrawal prefix events" series={withdrawals}/></div></section>
    <section className="space-y-3"><div><p className="citem-eyebrow">Observed routing surface</p><h3 className="text-sm font-semibold text-stone-200">Collector-visible population</h3></div><div className="grid gap-4 xl:grid-cols-2"><SeriesPanel title="Distinct prefixes observed" series={prefixes}/><SeriesPanel title="Distinct origin ASNs observed" series={origins}/></div></section>
