...
export function ProductLifecycle({projectId,report,versions,authoritativeVersion}:{projectId:string;report:Row;versions:Row[];authoritativeVersion:Row|null}){
 ...
 const flushDraft=async()=>{
  const saved=await requestAutosaveFlush();
  if(!saved)setMessage("Save the current report draft before continuing.");
  return saved;
 };
 return <div className="mt-6 space-y-6">
  <section className="card"><h2 className="text-xl font-semibold">Product metadata</h2><form className="mt-3 grid gap-3 sm:grid-cols-2" action={async fd=>{if(await flushDraft())await act(updateProductMetadata(projectId,String(report.id),{},fd));}}>
  ...
  <section className="card"><h2 className="text-xl font-semibold">Create version</h2><p className="text-sm text-slate-400">Snapshots the saved working draft and its analytical references without publishing it.</p><form className="mt-3 grid gap-3" action={async fd=>{if(await flushDraft())await act(createReportVersion(projectId,String(report.id),{},fd));}}>
