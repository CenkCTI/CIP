...
export const adapterResultSchema = z.discriminatedUnion("status", [
  z.object({ status: z.literal("SUCCEEDED"), items: z.array(normalizedCandidateSchema).max(1000), nextCursor: z.string().max(8000).optional() }).strict(),
  z.object({ status: z.literal("NOT_MODIFIED"), items: z.tuple([]) }).strict(),
]);
