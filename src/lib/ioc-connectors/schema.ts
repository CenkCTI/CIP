...
const adapterDiagnosticsSchema = z.object({ received_count:diagnosticCount,eligible_count:z.number().int().min(0).max(1000),already_seen_count:diagnosticCount,deferred_count:diagnosticCount.default(0),has_more:z.boolean().default(false),mapped_count:z.number().int().min(0).max(1000),mapping_skipped_count:z.number().int().min(0).max(1000),skip_reason_counts:z.partialRecord(z.enum(providerSkipReasons),z.number().int().min(1).max(1000))}).strict().superRefine((d,ctx)=>{if(d.received_count!==d.already_seen_count+d.eligible_count+d.deferred_count)ctx.addIssue({code:"custom",message:"Inconsistent received count."});if(d.eligible_count!==d.mapped_count+d.mapping_skipped_count)ctx.addIssue({code:"custom",message:"Inconsistent eligible count."});if(Object.values(d.skip_reason_counts).reduce((a,b)=>a+(b??0),0)!==d.mapping_skipped_count)ctx.addIssue({code:"custom",message:"Inconsistent skip reasons."});if(d.has_more!== (d.deferred_count>0))ctx.addIssue({code:"custom",message:"Inconsistent deferred state."})});
