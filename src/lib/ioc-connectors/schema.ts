...
const adapterDiagnosticsSchema = z.object({ received_count: z.number().int().min(0).max(1000), eligible_count: z.number().int().min(0).max(1000), already_seen_count: z.number().int().min(0).max(1000), mapped_count: z.number().int().min(0).max(1000), mapping_skipped_count: z.number().int().min(0).max(1000), skip_reason_counts: z.partialRecord(z.enum(providerSkipReasons), z.number().int().min(1).max(1000)) }).strict().superRefine((d, ctx) => { if (d.received_count !== d.eligible_count + d.already_seen_count) ctx.addIssue({ code: "custom", message: "Inconsistent received count." }); if (d.eligible_count !== d.mapped_count + d.mapping_skipped_count) ctx.addIssue({ code: "custom", message: "Inconsistent eligible count." }); if (Object.values(d.skip_reason_counts).reduce((a,b)=>a+(b??0),0) !== d.mapping_skipped_count) ctx.addIssue({ code: "custom", message: "Inconsistent skip reasons." }); });
export const adapterResultSchema = z.discriminatedUnion("status", [
  z.object({ status: z.literal("SUCCEEDED"), items: z.array(z.union([normalizedCandidateSchema, z.object({ provider_skip_reason: z.enum(providerSkipReasons) }).strict()])).max(1000), nextCursor: z.string().max(1000).optional(), diagnostics: adapterDiagnosticsSchema }).strict(),
  z.object({ status: z.literal("NOT_MODIFIED"), items: z.tuple([]), nextCursor: z.string().max(1000).optional(), diagnostics: adapterDiagnosticsSchema.refine(d=>d.eligible_count===0&&d.mapped_count===0&&d.mapping_skipped_count===0,"Modified diagnostics for NOT_MODIFIED.") }).strict(),
]);
