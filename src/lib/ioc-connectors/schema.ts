...
export const adapterResultSchema = z.discriminatedUnion("status", [
  z.object({ status: z.literal("SUCCEEDED"), items: z.array(z.union([normalizedCandidateSchema, z.object({ provider_skip_reason: z.enum(providerSkipReasons) }).strict()])).max(1000), nextCursor: z.string().max(8000).optional(), diagnostics: z.object({ received_count: z.number().int().min(0).max(1000), mapped_count: z.number().int().min(0).max(1000), mapping_skipped_count: z.number().int().min(0).max(1000), skip_reason_counts: z.partialRecord(z.enum(providerSkipReasons), z.number().int().min(1).max(1000)) }).strict().optional() }).strict(),
  z.object({ status: z.literal("NOT_MODIFIED"), items: z.tuple([]), diagnostics: z.object({ received_count: z.number().int().min(0), mapped_count: z.literal(0), mapping_skipped_count: z.number().int().min(0), skip_reason_counts: z.partialRecord(z.enum(providerSkipReasons), z.number().int().min(1).max(1000)) }).strict().optional() }).strict(),
]);
