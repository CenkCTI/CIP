...
liveDescribe("CİTEM NODE-6 real RIPE integration", () => {
  ...
  it("consumes a real RIPE RIS Live routing head through the NODE-6.3 status contract",async()=>{
    ...
    expect(response.data.stream.heartbeatFreshness).toBe("FRESH");expect(response.data.stream.latestSessionStatus).toBe("STREAMING");expect(response.data.stream.messagesObserved).not.toBeNull();expect(response.data.stream.segmentsPersisted).not.toBeNull();expect(BigInt(response.data.stream.messagesObserved??"0")).toBeGreaterThan(0n);expect(BigInt(response.data.stream.segmentsPersisted??"0")).toBeGreaterThan(0n);expect(response.data.stream.latestSourceObservedAt).not.toBeNull();expect(response.data.stream.latestNodeReceivedAt).not.toBeNull();
    const latest=response.data.latest;expect(latest).not.toBeNull();if(!latest)throw new Error("Expected a current routing head from real RIPE RIS Live traffic");expect(latest.acquisitionBasis).toBe("LIVE_STREAM");expect(latest.acquisitionChannel).toBe("RIS_LIVE_WEBSOCKET");expect(BigInt(latest.updateMessages)).toBeGreaterThan(0n);expect(latest.rrcCount).toBeGreaterThan(0);expect(latest.captureProfileKey).not.toBeNull();expect(latest.captureProfileVersion).not.toBeNull();expect(latest.captureProfileRrcCount??0).toBeGreaterThan(0);expect(["COMPLETE","PARTIAL","DEGRADED"]).toContain(latest.liveCollectionCoverage);expect(["AVAILABLE","PARTIAL"]).toContain(latest.dataAvailability);
