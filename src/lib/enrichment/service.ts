...
async function failKnownRun(context: Context, runId: string, error: EnrichmentError) {
  const failed = await context.supabase
    .from("enrichment_runs")
    .update({
      status: "FAILED",
    ...
    .select("id")
    .single<RunRow>();
  if (failed.error || !failed.data) throw mapEnrichmentDatabaseError(failed.error);
}

const MIN_STALE_RUN_MS = 5 * 60 * 1000;
const MAX_STALE_RUN_MS = 30 * 60 * 1000;

export function staleRunThresholdMs(providerTimeoutMs: number) {
  return Math.min(
    MAX_STALE_RUN_MS,
    Math.max(MIN_STALE_RUN_MS, providerTimeoutMs * 2),
  );
}

export function isStaleEnrichmentRun(
  run: { requested_at: string; updated_at?: string | null },
  providerTimeoutMs: number,
  now = Date.now(),
) {
  const activityAt = new Date(run.updated_at || run.requested_at).getTime();
  return Number.isFinite(activityAt) && now - activityAt >= staleRunThresholdMs(providerTimeoutMs);
}
...
export async function executeIndicatorEnrichment(
  ...
  try {
    ...
    const { data: latest, error: latestError } = await context.supabase
      .from("enrichment_runs")
      .select("id,requested_at,updated_at,status")
      .eq("project_id", context.projectId)
    ...
    if (latest && ["PENDING", "RUNNING"].includes(String(latest.status))) {
      if (!isStaleEnrichmentRun(latest, provider.requestTimeoutMs)) {
        return {
          ok: false,
          code: "ACTIVE_RUN",
      ...
      try {
        await failKnownRun(
          ...
          new EnrichmentError(
            "STALE_RUN",
            "The previous enrichment run exceeded its safe execution window and was closed.",
          ),
        );
        ...
        return {
          ok: false,
          code: "RUN_STATE_PERSISTENCE",
          error: "The stale enrichment run could not be closed safely. Retry after checking database availability.",
        };
      }
    }
    if (latest && !recoveredStaleRun && config.cooldownSeconds > 0) {
    ...
    const running = await context.supabase
      ...
      .select("id")
      .single<RunRow>();
    if (running.error || !running.data) throw mapEnrichmentDatabaseError(running.error);
    ...
    try {
      ...
      const completed = await context.supabase
        ...
        .select("id")
        .single<RunRow>();
      if (completed.error || !completed.data) throw mapEnrichmentDatabaseError(completed.error);
      ...
      try {
        ...
        return {
          ...
          runId,
          code: "RUN_STATE_PERSISTENCE",
          error: "The final enrichment run state could not be saved. Retry after checking database availability; the run may require stale-run recovery.",
        };
      }
      revalidatePath(`/projects/${context.projectId}/indicators/${indicatorId}`);
