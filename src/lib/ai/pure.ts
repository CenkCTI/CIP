  ...
  return [
    {
      role: "system" as const,
      content:
        "You are a CTI translation assistant. You have no tools, database, filesystem, browser, secrets, or write capability. Translate only the delimited source text. Preserve protected technical tokens exactly. Return exactly one JSON object matching the translation contract. Do not include markdown.",
    },
    {
      role: "user" as const,
      content: `${workflowContractText("translate_document")}
  ...
  return [
    {
      role: "system" as const,
      content:
        "Repair the translation JSON only. Do not add facts. Do not output server-owned fields. Preserve protected tokens exactly. Return exactly one JSON object matching the translation contract.",
    },
    {
      role: "user" as const,
      content: `${workflowContractText("translate_document")}
...
export { normalizeObservedIndicatorValue };

export function validateExtractedIndicator(s: {
  value: string;
  type: string;
}) {
  const result = validateObservedIndicator(s);
  return {
...
export function protectedTokens(text: string) {
  return Array.from(
    new Set(
      text.match(
        /(?:CVE-\d{4}-\d{4,}|T\d{4}(?:\.\d{3})?|https?:\/\/\S+|[\w.+-]+@[\w.-]+\.[A-Za-z]{2,}|\b[a-fA-F0-9]{32,64}\b|HKEY_[A-Z_\\\w.-]+|\b\d{1,3}(?:\.\d{1,3}){3}\b)/g,
      ) ?? [],
    ),
  );
}

export function missingProtectedTokens(source: string, translated: string) {
  return protectedTokens(source).filter((token) => !translated.includes(token));
}

export function sourceTextForEntityExtraction(sourceData: unknown) {
  const data = sourceData as {
    pastedText?: unknown;
    notes?: { title?: unknown; content?: unknown }[];
    evidence?: { title?: unknown; description?: unknown }[];
  };
  const parts = [String(data?.pastedText ?? "")];
  for (const note of data?.notes ?? []) {
    parts.push(`${String(note.title ?? "")}\n${String(note.content ?? "")}`);
  }
...
function cleanEntityName(name: string) {
  return name
    .replace(
      /\s+(?:is|uses|and|also|references|associated|with|vulnerability|identifier|fictional|internal|synthetic)\b.*$/i,
      "",
    )
    .trim();
}
...
export function extractExplicitEntityCandidates(text: string) {
  const candidates: {
    entity_type: "actors" | "malware" | "campaigns" | "cves";
    name: string;
  ...
  for (const match of text.matchAll(/\bcampaign\s+(?:named\s+)?([A-Z][A-Za-z0-9-]*(?:\s+[A-Z][A-Za-z0-9-]*){0,4})/gi)) {
    candidates.push({
      entity_type: "campaigns",
      name: cleanEntityName(match[1]),
      confidence: "LOW",
      evidence_excerpt: excerpt,
      caveats: ["Explicitly named project candidate; not externally verified."],
      source_ref: null,
    });
  }
  for (const match of text.matchAll(/\bthreat actor\s+(?:named\s+)?([A-Z][A-Za-z0-9-]*(?:\s+[A-Z][A-Za-z0-9-]*){0,4})/gi)) {
    candidates.push({
      entity_type: "actors",
      name: cleanEntityName(match[1]),
      confidence: "LOW",
  ...
  for (const match of text.matchAll(/\bmalware\s+(?:named\s+)?([A-Z][A-Za-z0-9-]*(?:\s+[A-Z][A-Za-z0-9-]*){0,3})/gi)) {
    candidates.push({
      entity_type: "malware",
      name: cleanEntityName(match[1]),
      confidence: "LOW",
  ...
  for (const match of text.matchAll(/\b(CVE-\d{4}-\d{4,})\b/g)) {
    candidates.push({
      entity_type: "cves",
      name: match[1],
      confidence: "LOW",
      evidence_excerpt: excerpt,
      caveats: [
        "Syntactically valid CVE candidate from source; not externally verified.",
      ],
      source_ref: null,
    });
  }
  return uniq(
    candidates.filter((item) => /^[A-Z]/.test(item.name)),
    (item) => `${item.entity_type}:${item.name.toLowerCase()}`,
  ).slice(0, 25);
}

export function buildEntityExtractionRepairPrompt(
  sourceText: string,
  malformedOutput: string,
  issues: string[],
) {
