...
export async function ingestStoredResearchFeed(projectId:string,feedId:string) {
 ...
 let stage:FeedDiagnosticStage="REQUEST";
 try {const response=await fetchFeed(run.configured_url,{etag:run.etag,lastModified:run.last_modified},{diagnostic:{feedSourceId:feedId,fetchRunId:run.run_id}});let type:"UNKNOWN"|"RSS"|"ATOM"|"JSON_FEED"="UNKNOWN";let items:Record<string,unknown>[]=[];if(response.status===200){stage="PARSE";const parsed=parseFeed(response.body,response.finalUrl);type=parsed.type;items=parsed.items.map(i=>{const f=itemFingerprints(i);return{external_id:i.externalId,title:i.title,canonical_url:i.canonicalUrl,summary_text:i.summaryText,content_text:i.contentText,author_name:i.authorName,published_at:i.publishedAt,source_updated_at:i.sourceUpdatedAt,language:i.language,categories:i.categories,url_hash:f.url,content_hash:f.content};});}
  stage="FINALIZE";const {error:completeError}=await completeFeedWorkflow({p_actor_id:ctx.user.id,p_project_id:projectId,p_feed_source_id:feedId,p_run_id:run.run_id,p_lease_token:run.lease_token,p_request_url_hash:run.request_url_hash,p_status:response.status===304?"NOT_MODIFIED":"SUCCEEDED",p_feed_type:type,p_final_url:response.finalUrl,p_http_status:response.status,p_response_bytes:response.bytes,p_etag:String(response.headers.etag??"")||null,p_last_modified:String(response.headers["last-modified"]??"")||null,p_items:items});if(completeError){logFeedDiagnostic({stage,error:completeError,code:"INTERNAL_ERROR",feedSourceId:feedId,fetchRunId:run.run_id});throw new FeedError("INTERNAL_ERROR");}return{success:response.status===304?"Feed checked; content was not modified.":`Feed fetched; ${items.length} items processed.`};
