...
export const editFeedFormSchema=z.object({name:z.string().trim().min(1).max(240),description:z.string().trim().max(10000).default(""),configured_url:z.union([safeUrl,z.literal("")]),enabled:z.boolean()}).strict();
