...
insert into public.indicators(id,project_id,value,type)values('30000000-0000-4000-8000-000000000005','20000000-0000-4000-8000-000000000001','198.51.100.10','IP'),('30000000-0000-4000-8000-000000000007','20000000-0000-4000-8000-000000000001','https://Example.com/CasePath?a=UP','URL'),('30000000-0000-4000-8000-000000000008','20000000-0000-4000-8000-000000000001','https://Example.com/casepath?a=UP','URL'),('30000000-0000-4000-8000-000000000009','20000000-0000-4000-8000-000000000001','C:/Windows/System32/cmd.exe','FILE'),('30000000-0000-4000-8000-000000000010','20000000-0000-4000-8000-000000000001','HKLM\Software\Example','REGISTRY'),('30000000-0000-4000-8000-000000000011','20000000-0000-4000-8000-000000000001','https://user:pass@example.com/legacy','URL');
...
do $$declare standalone uuid; inv jsonb; inv_id uuid; counts jsonb; item uuid; excluded uuid; removed uuid;begin
 ...
 begin perform public.add_explicit_intel_profile_item('10000000-0000-4000-8000-000000000001',standalone,'INDICATOR','https://:',null,'URL');raise exception 'empty url host accepted';exception when invalid_parameter_value then null;end;
 begin perform public.add_explicit_intel_profile_item('10000000-0000-4000-8000-000000000001',standalone,'INDICATOR','https://example.com:bad/path',null,'URL');raise exception 'bad url port accepted';exception when invalid_parameter_value then null;end;
 ...
 if(select count(*) from public.intel_profile_items where profile_id=inv_id and origin='DERIVED' and state='ACTIVE')<>8 then raise exception 'deterministic seed count failed';end if;if (inv->'refresh'->>'skipped')::int<>3 then raise exception 'unsupported/malformed indicator skip count failed';end if;if exists(select 1 from public.intel_profile_items where profile_id=inv_id and source_entity_id in('30000000-0000-4000-8000-000000000009','30000000-0000-4000-8000-000000000010','30000000-0000-4000-8000-000000000011')) then raise exception 'unsupported or malformed indicator was seeded';end if;if not exists(select 1 from public.intel_profile_items where profile_id=inv_id and source_entity_id='30000000-0000-4000-8000-000000000007' and indicator_type='URL' and normalized_value='https://example.com/CasePath?a=UP') then raise exception 'seed did not preserve indicator type/url case';end if;
 ...
 if (counts->>'skipped')::int<>3 then raise exception 'refresh skip count was not deterministic';end if;
