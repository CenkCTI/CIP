...
# Runs every repository migration, including 021, inside one real PostgreSQL
# transaction. Supabase-provided auth/storage objects are minimally stubbed.
DB_NAME="citem_phase2_1e_${$}"
...
begin
 ...
 begin insert into attribution_evidence_evaluations(project_id,campaign_id,hypothesis_id,evidence_item_id,impact,diagnostic_value,rationale,created_by) values(p,c2,h,e,'SUPPORTS','HIGH','Cross campaign',u); raise exception 'cross campaign accepted'; exception when foreign_key_violation then null; end;
 insert into attribution_evidence_evaluations(project_id,campaign_id,hypothesis_id,evidence_item_id,impact,diagnostic_value,rationale,created_by) values(p,c,h,e,'SUPPORTS','HIGH','Distinguishing evidence',u) returning id into ev;
...
echo "Phase 2.1E migration transaction smoke test passed."
