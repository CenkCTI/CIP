...
Deterministic Investigation seeding skips unsupported existing Indicator subtypes (`FILE`, `REGISTRY`) and legacy malformed supported Indicator values on a per-record basis. These records are not converted to other subtypes, not guessed from their values, and not inserted as keywords; each safe skip increments the `skipped` count while valid supported records continue to seed atomically.
