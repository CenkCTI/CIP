...
OTX Search & Select is the default analyst workflow. An authenticated owner searches the fixed read-only `GET /api/v1/search/pulses` endpoint with a 2–120 character term; the server starts with `page=1` and `limit=5`, decrypts the owner credential, and returns up to 25 bounded previews only through explicit Load more requests without persistence or an ingestion run. Search results never import automatically. Selecting **Import Pulse** causes the server to validate the 24-hex ID and refetch both the fixed Pulse detail and Pulse-indicator endpoints before trusted ingestion.
...
### Global catalogue search and exact observable lookup

The connected OTX workflow searches the global Pulse catalogue directly; it never downloads subscribed Pulses, applies bootstrap look-back settings, searches local candidates, or requires a previous bulk synchronization. Free-text searches call only `GET https://otx.alienvault.com/api/v1/search/pulses` with the trimmed query, an analyst-selected page from 1 through 5, and `limit=5`. **Load more** requests exactly one next page, up to 25 total previews. Results remain transient and require explicit Pulse import.

Exact IPv4, IPv6, domain/hostname, URL, MD5, SHA1, SHA256, and CVE inputs use the fixed OTX indicator `general` endpoint instead. CİTEM displays bounded reputation/general context, safe references, and containing Pulse IDs; it never automatically imports the observable. Search failures expose only bounded elapsed milliseconds, HTTP status when available, timeout category, and response byte count under `OTX_SEARCH_TIMEOUT`, `OTX_SEARCH_RATE_LIMITED`, `OTX_SEARCH_AUTH_FAILED`, `OTX_SEARCH_NETWORK_FAILURE`, or `OTX_SEARCH_INVALID_RESPONSE`. Search terms, API keys, Pulse/IOC content, bodies, headers, and stack traces are never logged or returned.
