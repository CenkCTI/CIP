...
## Migration 018 hardening

Apply migration 018 after 017, then run `NOTIFY pgrst, 'reload schema';`:

```bash
psql "$SUPABASE_DB_URL" -f supabase/migrations/202607290018_phase2_1b_enrichment_hardening.sql
```

Migration 018 preserves existing rows and owner-scoped reads. Results are
append-only, run deletion is rejected, run identity/snapshot fields are immutable,
and terminal runs cannot change. Only `PENDING → RUNNING/FAILED/CANCELLED` and
`RUNNING → SUCCEEDED/PARTIAL/FAILED/CANCELLED` are accepted. Result inserts must
match the project and Indicator of a currently `RUNNING` run.

The authenticated server flow retains run INSERT/UPDATE and result INSERT. Since it
uses the user's Supabase session, a technically capable same-owner client can attempt
the remaining allowed writes. Triggers constrain their shape and lifecycle, but
complete prevention requires a stronger trusted-server boundary; none is claimed.
...
Before blocking on an active run, the server checks its latest activity against
`max(provider timeout × 2, 5 minutes)`, capped at 30 minutes. A stale `PENDING` or
`RUNNING` run becomes `FAILED` with `STALE_RUN`, prior results remain untouched, and
a new run may start. Failure to persist recovery or a final state returns the safe,
actionable `RUN_STATE_PERSISTENCE` error.
...
- Browser acceptance requires migrations 017 and 018 on the configured Supabase project and fixture environment flags on the deployed server.
