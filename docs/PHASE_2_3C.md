...
The adapter uses official `lastModStartDate`, `lastModEndDate`, `startIndex`, and `resultsPerPage` parameters. Initial lookback defaults to 24 hours and is capped at 7 days. Successful cursors use a five-minute overlap and never request more than the official 120-day date range. A run is capped at 20 pages and 2,000 records; each page is capped at 8 MiB. `NVD_API_KEY` is optional, server-only, never stored, never rendered, and never logged.
...
Migration 033 has not been applied to Preview, Production, or another remote Supabase project. After merge and explicit operator authorization:

1. Apply migration 033 and reload PostgREST schema.
2. Enable `TECHINT_TEST_SOURCE_ENABLED=true` in Preview only.
3. Open `/techint/sources`, enable the synthetic source, and run it twice.
4. Confirm canonical signal idempotency, run counters, history, second-user isolation, lifecycle/audit behavior, and no analytical side effects.
5. Disable the synthetic gate and redeploy.
6. Enable CISA KEV and NVD only after controlled Preview review; keep NVD lookback bounded and intervals conservative.
7. Inspect sanitized logs and confirm no API key or raw body appears.
8. Repeat essential checks in Production only after explicit Production authorization.

Rollback is application rollback plus pausing sources. Collection history and source-backed signals are preserved; migration 033 is additive and is not destructively rolled back in a live database.
